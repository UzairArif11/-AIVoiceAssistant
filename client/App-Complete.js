...
export default function App() {
  ...
  const startVoiceService = async () => {
    if (!VoiceService) {
      Alert.alert('Error', 'Voice service not available. Make sure you are using a development build.');
      return;
    }
          ...
          <View style={styles.welcomeContainer}>
            ...
            {!VoiceService && (
              <Text style={styles.warningText}>
                ⚠️ Voice service not available - using development build is required for voice features
              </Text>
            )}
          </View>
        ) : (
